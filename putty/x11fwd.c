...
void x11_free_fake_auth(struct X11FakeAuth *auth)
{
	...
	struct XDMSeen *seen;
	while ((seen = (struct XDMSeen *)delpos234(auth->xdmseen, 0)) != NULL)
	    sfree(seen);
                              ...
                              struct X11FakeAuth **auth_ret)
{
    ...
    if ((auth = (struct X11FakeAuth *)find234(authtree, &match_dummy, 0)) == NULL)
	...
	ret = (struct XDMSeen *)add234(auth->xdmseen, seen);
	if (ret != seen) {
	    sfree(seen);
	...
	for (;;) {
	    seen = (XDMSeen *)index234(auth->xdmseen, 0);
	    assert(seen != NULL);
 ...
 */
struct X11Connection *x11_init(void* frontend, tree234 *authtree, void *c,
...
int x11_send(struct X11Connection *xconn, char *data, int len)
{
	 ...
	 */
        ...
        sk_write(xconn->s, (const char*)greeting, greeting_len);
                        ...
                        int *outlen)
{
    ...
    if (auth_proto == X11_MIT) {
        authdata = (const unsigned char*)auth_data;
