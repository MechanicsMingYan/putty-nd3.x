			 ...
			 struct Packet *pktin)
{
	...
	if ((ssh->username = get_remote_username(ssh->conf)) == NULL) {
		...
		ssh->send_ok = 1;
		crWaitUntil(!pktin);
		ret = get_userpass_input(ssh->frontend, s->cur_prompt, in, inlen);
		ssh->send_ok = 0;
	    }
	    if (!ret) {
		/*
...
static void ssh_setup_portfwd(Ssh ssh, Conf *conf)
{
	...
	if (epf->status == CREATE) {
	    ...
	    if (epf->type == 'L') {
                char *err = pfl_listen(ssh->frontend, epf->daddr, epf->dport,
                                       epf->saddr, epf->sport,
                                       ssh, conf, &epf->local,
                                       epf->addressfamily);
		...
		char *err = pfl_listen(ssh->frontend, NULL, -1, epf->saddr, epf->sport,
                                       ssh, conf, &epf->local,
                                       epf->addressfamily);
...
static void ssh1_smsg_x11_open(Ssh ssh, struct Packet *pktin)
{
	...
	c->u.x11.xconn = x11_init(ssh->frontend, ssh->x11authtree, c, NULL, -1);
        c->remoteid = remoteid;
        c->halfopen = FALSE;
...
static void ssh1_msg_port_open(Ssh ssh, struct Packet *pktin)
{
	...
	logeventf(ssh, "Received remote port open request for %s:%d",
		  pf.dhost, port);
	err = pfd_connect(ssh->frontend, &c->u.pfd.pf, pf.dhost, port,
...
static void ssh2_msg_channel_open(Ssh ssh, struct Packet *pktin)
{
	...
	if (!ssh->X11_fwd_enabled && !ssh->connshare)
	    error = "X11 forwarding is not enabled";
            ...
            c->u.x11.xconn = x11_init(ssh->frontend, ssh->x11authtree, c,
                                      addrstr, peerport);
	    c->type = CHAN_X11;
	...
	if (realpf == NULL) {
	    error = "Remote port is not recognised";
            ...
            err = pfd_connect(ssh->frontend, &c->u.pfd.pf, realpf->dhost, realpf->dport,
			     ...
			     struct Packet *pktin)
{
	...
	if (s->got_username && !conf_get_int(ssh->conf, CONF_change_username)) {
	    ...
	    if (!ret) {
		/*
		 * get_userpass_input(ssh->frontend, ) failed to get a username.
		 * Terminate.
		 */
		free_prompts(s->cur_prompt);
		ssh_disconnect(ssh, "No username provided", NULL, 0, TRUE);
	...
	while (1) {
		...
		while (!key) {
			...
			while (ret < 0) {
			    ssh->send_ok = 1;
			    crWaitUntilV(!pktin);
			    ret = get_userpass_input(ssh->frontend, s->cur_prompt,
						     in, inlen);
			    ssh->send_ok = 0;
			}
			if (!ret) {
