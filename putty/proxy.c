...
#define do_proxy_dns(conf) \
    (conf->proxy_dns == FORCE_ON || \
	 (conf->proxy_dns == AUTO && \
	      conf->proxy_type != PROXY_SOCKS4))
                           ...
                           int port, Conf *conf)
{
    ...
    exclude_list = conf->proxy_exclude_list;
		      ...
		      Plug plug, Conf *conf)
{
    ...
    pplug->frontend = ((plug_frontend*)plug)->frontend;
	pplug->fn = &plug_fn_table;
	pplug->proxy_socket = ret;

	/* look-up proxy */
	proxy_addr = sk_namelookup(conf->proxy_host,
...
{
    char *fmt = conf->proxy_telnet_command;
    char *ret = NULL;
...
#define ENSURE(n) do { \
	...
	if (fmt[so] == '\\') {
		...
		char *username = conf->proxy_username;
		...
		char *password = conf->proxy_password;
